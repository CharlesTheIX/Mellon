...
pub const Base64 = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, io: *IO, Err: *ErrorHandler) Base64 {
        const symbols = "+/";
    ...
    pub fn controller(self: *Base64, args: []const u8) void {
        ...
        switch (func) {
            .Encode => return try self.encode(input),
            .Decode => return try self.decode(input),
            .Help => return self.help() catch |err| return self.Err.handle(
                err,
                "Failed to show base64 encoder/decoder help\n\n",
                false,
                true,
            ),
