...
pub fn readFile(path: []const u8) []const u8 {
    ...
    const abs_path: []const u8 = getAbsPath(path);
...
fn getAbsPath(path: []const u8) []const u8 {
    ...
    if (std.mem.eql(u8, path_first_char, "~")) {
        const env = std.process.getEnvMap(allocator) catch return "";
        const home_path = env.get("HOME") orelse "";
        return std.fs.path.join(allocator, &[_][]const u8{ home_path, "/", path[1..] }) catch "";
    }
    ...
    while (path_parts.next()) |part| {
        if (std.mem.eql(u8, part, ".")) {
            count += 2;
            cwd_path = std.fs.path.join(allocator, &[_][]const u8{ cwd_path, "." }) catch return "";
        } else if (std.mem.eql(u8, part, "..")) {
            if (count == 0) cwd_path = std.fs.path.join(allocator, &[_][]const u8{ cwd_path, ".." }) catch return "";
            if (count >= 2) cwd_path = std.fs.path.join(allocator, &[_][]const u8{ cwd_path, "/.." }) catch return "";
            count += 3;
        } else break;
    }

    const cwd = std.fs.cwd().realpathAlloc(allocator, cwd_path) catch return "";
    defer allocator.free(cwd);
    return std.fs.path.join(allocator, &[_][]const u8{ cwd, "/", path[count..] }) catch return "";
}
